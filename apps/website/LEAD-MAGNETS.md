...
17 September technical verification passed with `npm run build`, `npm test` (29 tests), `npm run check` against the local Worker (20 routes and six unchanged download hashes), `scripts/check-lead-magnets-browser.mjs`, `scripts/check-scorecard-journey-browser.mjs` and `scripts/check-homepage-browser.mjs`. The checks covered the required contextual entry, keyboard and mobile use, contact-back retention, refresh clearing, no browser storage and no assessment submission. Live publication was then checked separately as recorded below.

**Live deployment verification — 17 September 2026:** Cloudflare Worker version `c24bdd9d-095b-440e-bc6c-8d5dbfc41c67` was deployed to https://heutrix-labs-original-theme.janith.workers.dev/. The live Worker passed 20 route checks, all six unchanged download hashes, the full scorecard journey and the desktop/mobile resource journey. No browser check submitted an assessment or request.
...
Scorecard refinements are on local branch `scorecard-journey-refinements`; they are not deployed. Drafts remain in React memory, keyed by tool and allowlisted workflow entry, during normal navigation. Refreshing or closing the tab clears them. Only `onboarding`, `payment`, `reporting` and `action-tracking` are accepted workflow URL values; answers never enter URLs or browser storage. Existing scoring weights, thresholds and conservative overrides are retained. Signal labels and deterministic actions use the actual answers; method version is `2026-09-11`.

Verification: `npm.cmd run build`, `npm.cmd test` (29 tests), `npm.cmd run check` with the local Worker (20 routes and six unchanged download hashes), `scripts/check-lead-magnets-browser.mjs`, and `scripts/check-scorecard-journey-browser.mjs` passed. The latter covers contextual entry, keyboard use, 320/390px mobile entry, low signals, evidence/feasibility branches, contact-back retention and refresh clearing. Screenshots are in `output/playwright/scorecard/`. No form requests were sent during these browser checks.

See [CTA-STYLE-GUIDE.md](CTA-STYLE-GUIDE.md). The release notes below are historical, including mock-mode and earlier provider references.
