...
The active implementation is reconciled in `apps/website/`. See the [integration record](../changes/2026-09-08-workspace-integration.md). Current website CTAs are **Talk to Heutrix**, **See How It Works** and **Start the Scorecard**; enquiry/referral forms prepare email drafts, not submissions or bookings. **Heutrix AI Guardrails** remains the product name. Three illustrative examples are displayed; the controlled proof collection and its approvals remain intact. Earlier page prose below is a retained drafting baseline where it differs from this implementation; do not paste it over current application copy. Follow the CTA guide and integration record when preparing a page revision.
...
**12 September outcome revision — local, not published:** The introduction, decision table and product outcome wording below align with `D:/AI Information Heirarchy/heutrix-website`. The 8 September integration note is historical; remaining page prose retains its drafting-baseline status.

## Diagnose the right problem, transform one bounded workflow, or put practical AI guardrails in place.

For Australian disability-provider owners and operations leaders, with allied health a secondary fit for comparable workflows. If staff keep chasing updates or checking who acts next, start with the workflow causing that repeated work.
...
|---|---|---|
| “We know operations are messy, but not what to fix first.” | Heutrix Diagnostics | A workflow map and ranked options to decide what to address first, including when not to build |
| “We know which workflow needs to work better.” | Heutrix Workflow Transformation | A tested workflow showing the next action, its owner and the exception path, with instructions for the receiving team |
| “Staff use AI, but our rules and review controls are unclear.” | Heutrix AI Guardrails | Use and information rules that show staff when to proceed, seek human review or stop |

## Heutrix Diagnostics

### See what is slowing the work down and what to fix first.

When teams disagree about why work stalls, Heutrix Diagnostics maps the workflow, ownership gaps and dependencies, then ranks feasible options in a paid, standalone implementation brief.
...
Use the brief to decide which bottleneck to address, what needs resolving before implementation and when not to build. It remains useful if you act internally or choose another implementer. Its fee is not automatically credited to later implementation. Approved artefacts may be reused so the same discovery is not repeated.
...
### See the next action, its owner and what is waiting.

When intake, reporting or document work depends on repeated chasing, Heutrix Workflow Transformation defines the steps, owners, review points and exception path, then implements and tests that one non-clinical workflow in approved systems.
...
Staff can find the next action and route blocked work to the agreed reviewer. The receiving team gets tested steps, staff and administrator guidance, agreed access and known limitations so it can operate the workflow after handover.
...
Heutrix AI Guardrails is a paid governance engagement for organisations considering or already using commercially available AI tools.
...
Instead of guessing whether an AI task is suitable, staff can consult the documented use and information rules, identify the accountable reviewer and follow the escalation path when a use is unclear.
...
Use the online **Workflow Bottleneck Scorecard** to assess one workflow through 10 required questions across 3 steps, then see your result immediately. The separate Excel workbook compares up to 12 workflows.
...
## How current delivery evidence maps to the products

**Publication approval — implementation note, not public copy:** Janith cleared the current anonymised case studies on 5 September 2026. Render approved wording with evidence limits and four-provider disclosure. No identifying source material or stronger claims are authorised.

### Heutrix Workflow Transformation

The evidence-refreshed collection documents Heutrix delivery for one anonymised Australian disability support provider across lifecycle mapping, worker onboarding, controlled invoice workbooks, reminders, invoice status, discrepancy handling, Power BI reporting, recurring data snapshots and incident-management workflows.

Selected client-feedback-supported outcomes include approximately 1–4 hours less onboarding administration per worker, approximately 60% fewer out-of-cycle payment events, and up to approximately two hours less investigation per discrepancy. These are approximate, not independently audited, and are not promised outcomes for a future engagement.

**Editorial index:** [Heutrix case-study collection](../case-studies-and-assets/README.md)

### Heutrix Workflow Transformation with AI Guardrails

The shift-note case documents structured capture and a Heutrix-delivered summarisation automation used to assist human review. Client feedback supports practical operation, while the workflow configuration, model boundary, information path and representative output have not been independently reviewed. The case does not establish privacy, clinical, documentation-quality or safety outcomes.

**Editorial source:** [Shift-note capture and summarisation](../case-studies-and-assets/15-shift-note-capture-and-summarisation.md)

No case in the current collection should be presented as a standalone Heutrix Diagnostics engagement. The lifecycle-mapping evidence demonstrates a method used within delivery, not a separate Diagnostics client outcome.
