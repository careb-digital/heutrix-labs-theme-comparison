import { canonicalUrlForPath, normalizePath } from './siteUrl.js';
import {ResourceHub, InteractiveTool} from './leadMagnets/InteractiveTools';
import EnquiryPage from './leadMagnets/EnquiryPage';
...
import legacyCases from './legacyCasePaths.json';
...
const appRoutes = [...routes, ...caseStudies.map(c => ({path:'/case-studies/'+c.slug,label:c.title,seoTitle:c.title+' | Heutrix',metaDescription:'Illustrative provider workflow. '+c.metric}))];
const routeMap = new Map(appRoutes.map((route) => [route.path, route]));
const knownPaths = new Set(appRoutes.map((route) => route.path));
const legacyRedirects = new Map([
  ['/pricing', '/services#how-engagements-are-agreed'],
  ['/safe-ai', '/ai-guardrails']
]);
for (const path of legacyCases) legacyRedirects.set(path, '/case-studies');
const notFoundMeta = {
  seoTitle: 'Page Not Found | Heutrix',
  metaDescription: 'The requested Heutrix page could not be found.'
};
const fadeUp = {
...
function setMeta(page) {
  ...
  document.querySelector('meta[name="robots"]')?.setAttribute('content', page === notFoundMeta ? 'noindex, nofollow' : 'index, follow');
  ...
  for (const [attribute, name, content] of [
    ['property', 'og:title', page.seoTitle], ['property', 'og:description', page.metaDescription],
    ['property', 'og:url', canonicalUrl], ['name', 'twitter:title', page.seoTitle],
    ['name', 'twitter:description', page.metaDescription],
  ]) {
...
function App({initialLocation} = {}) {
  ...
  useEffect(() => {
    const handleClick = (event) => {
      ...
      if (event.defaultPrevented || anchor.hasAttribute('download') || isModifiedClick || anchor.target || !isSameOrigin || !knownPaths.has(path)) return;
  ...
  useEffect(() => {
    ...
    const actionStart = mainRef.current?.querySelector('[data-cta-entry]:not([hidden])');
    if (actionStart) {
      actionStart.scrollIntoView({ block: 'start', behavior: 'instant' });
      actionStart.focus({ preventScroll: true });
      return;
    }
    ...
    <MotionConfig reducedMotion="user">
      <div className="min-h-screen bg-surface font-body-lg text-on-surface antialiased selection:bg-secondary/30 selection:text-primary">
        ...
        <main id="main-content" ref={mainRef} tabIndex="-1" className="outline-none">
          <PageErrorBoundary key={location.path}><PageRenderer path={location.path} search={location.search} /></PageErrorBoundary>
        </main>
        <Footer currentPath={location.path} currentHash={location.hash} currentSearch={location.search} />
      </div>
    </MotionConfig>
  );
}
...
function PageRenderer({ path, search }) {
  if (path.startsWith('/resources/') && knownPaths.has(path)) return <InteractiveTool key={path} id={path.split('/')[2]} search={search} />;
  if (path.startsWith('/case-studies/') && knownPaths.has(path)) return <CaseStudyPage slug={path.split('/')[2]} />;
  switch (path) {
    ...
    case '/resources':
      return <ResourceHub />;
    case '/contact':
      return <EnquiryPage key={'consultation'+search} search={search} />;
    case '/refer': return <EnquiryPage key="referral" kind="referral" search={search} />;
...
function ButtonLink({ href, children, variant = 'primary', icon = 'arrow_forward', className = '' }) {
  const styles = {
    primary: 'theme-primary shadow-md',
    secondary: 'theme-secondary',
    mint: 'theme-primary shadow-md',
    quiet: 'theme-secondary'
  };
  ...
  return (
    ...
    >
      ...
      <div className="mb-lg flex flex-col gap-sm sm:flex-row sm:flex-wrap">
        <ButtonLink href={`/resources/${resource.id}`}>{resource.id === 'workflow-bottleneck-scorecard' ? 'Start the Scorecard' : 'Take the Assessment'}</ButtonLink>
        ...
        <DownloadLink href={resource.workbookHref} resourceId={resource.id} format="xlsx" variant="secondary">
          {resource.id === 'workflow-bottleneck-scorecard' ? 'Download the Excel Workbook' : 'Get the Template'}
        </DownloadLink>
      </div>
  ...
  return (
    <Section className="bg-surface-container-low overflow-hidden">
      <div className="grid gap-xxl lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        <motion.div variants={stagger} initial={false} whileInView="show" viewport={{ once: true, amount: 0.2 }}>
        ...
        <motion.div variants={fadeUp} initial={false} whileInView="show" viewport={{ once: true, amount: 0.2 }}>
    ...
    <>
      ...
      >
        ...
        <p>
          ...
          no-obligation introductory conversation. You do not need to select a product first.
        </p>
      </PageHero>
      ...
      <Section>
        <div className="grid gap-xl lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          <SectionIntro title={content.improvementsTitle}>
            <p>{content.improvementsLead || 'Heutrix focuses on practical workflow support that can be used in day-to-day operations and handed over clearly.'}</p>
          </SectionIntro>
          <div className="rounded-xl border border-outline-variant bg-white p-lg shadow-sm">
  ...
  return (
    <>
      ...
      >
        ...
        <p>
          Heutrix helps organisations turn general AI concerns into clear use cases, information rules, human review
  ...
  return (
    <>
      ...
      <Section>
        <div className="grid gap-lg lg:grid-cols-3">
          <InfoCard icon="no_accounts" title="No email gate">
            <p>
              Downloads are provided directly. If you later request an introductory conversation, that separate request follows the lead
      ...
      <CtaSection
        ...
        body="Bring a high-level, non-sensitive description to a 20-minute, no-obligation introductory conversation. The outcome may be Diagnostics, Workflow Transformation, AI Guardrails, more evidence gathering or no project."
        cta={{ label: 'Talk to Heutrix', href: '/contact?source=resources' }}
      />
    </>
  );
}
...
function PrivacyPage() {
  return (
    <>
      <PageHero title="Privacy policy and data handling" compact>
        <p>
          This policy explains how Heutrix Pty Ltd (ABN 64 702 109 662; ACN 702 109 662) handles personal information
          from this website, business enquiries, referrals and their follow-up. Contact us at{' '}
          <a className="underline" href="mailto:hello@heutrix.com.au">hello@heutrix.com.au</a>.
        </p>
        <p>
          We ask for the least information needed to understand a workflow problem. This website is for business
          enquiries; project-specific arrangements must be agreed before any real client records are used.
        </p>
      ...
      <Section>
        <div className="grid gap-lg lg:grid-cols-3">
          <InfoCard icon="contact_page" title="Information for the introductory conversation">
            <p>
            ...
            <p className="mt-md">
              You may provide your name, business email, organisation, role, service interest, sector, systems,
              workflow description, timing and preferred contact time. We also receive your permissions and any
              assessment summary you choose to include. Review your enquiry or referral first, then select Send
              Enquiry or Send Referral. Reviewing a draft does not send it. A successful submission confirms receipt
              of the request; a conversation time is agreed separately.
            </p>
          </InfoCard>
          <InfoCard icon="download" title="Resource downloads">
            <p>
              ...
              permission. If you separately request an introductory conversation from the resources page, the request may record that
          ...
          <InfoCard icon="person_add" title="Where information comes from">
            <p>
              We receive information directly from you through a form, email or conversation. A referrer may provide
              an organisation name and a business contact’s name or email with permission. Only introduce someone
              or share their details if you have that permission.
            </p>
            <p className="mt-md">
              You can browse and download resources without identifying yourself. To answer an enquiry or arrange
              a conversation, we need sufficient contact details; without them we may be unable to respond.
            </p>
          </InfoCard>
          <InfoCard icon="task_alt" title="Why we use the information">
            <p>
              We use enquiry information to respond, assess whether we can help, arrange a conversation, prepare
              requested follow-up and manage the resulting business relationship. We also use relevant records to
              handle privacy requests, resolve issues and meet applicable record-keeping obligations.
            </p>
            <p className="mt-md">
              An enquiry or download does not subscribe you to a marketing list. The website does not sell personal
              information or send assessment answers to an AI service.
            </p>
          </InfoCard>
          <InfoCard icon="verified_user" title="Access and safeguards">
            <p>
              Enquiry records are for authorised people handling your request and supporting the website and its
              communication services. We use service providers for these purposes and may disclose information
              where you authorise it or where the law requires it.
            </p>
            <p className="mt-md">
              Website traffic uses HTTPS. No online service can guarantee complete security, so keep sensitive
              records, credentials and unnecessary personal details out of public enquiries.
            </p>
          </InfoCard>
        </div>
      </Section>
      ...
      <Section className="bg-surface-container-low">
        <SectionIntro title="What happens to website information" className="mb-xl">
          <p>This website is operated by Heutrix Pty Ltd, ABN 64 702 109 662. These details describe the website enquiry and resource journey.</p>
        </SectionIntro>
        <div className="grid gap-lg md:grid-cols-2">
          <InfoCard icon="mail" title="Sending an enquiry or referral">
            <p>
              When you send, Formspree processes and stores your reviewed details and sends an email notification to
              hello@heutrix.com.au. The request includes your contact details, message, permissions and the page or
              resource you came from. An assessment summary is included only if you choose to share it.
            </p>
            <p className="mt-md">
              Formspree processes information in the United States and other countries where it operates. It may also
              collect technical information, such as your IP address and browser details, to operate and protect its
              service. Read <a className="underline" href="https://formspree.io/legal/privacy-policy/">Formspree’s privacy policy</a>.
            </p>
          </InfoCard>
          <InfoCard icon="edit_note" title="Drafts and assessment answers">
            <p>
              Unsent drafts and assessment answers stay in this page’s memory. They are not saved to browser storage
              or sent automatically. Reloading or closing the page clears them. Discarding a draft or successfully
              sending it clears that form’s browser draft; it does not delete a request already held by Formspree or email.
            </p>
            <p className="mt-md">Only introduce an organisation or share another person’s business contact details with their permission.</p>
          </InfoCard>
          <InfoCard icon="language" title="Hosting and measurement">
            <p>
              Cloudflare hosts and delivers this website and processes technical connection information, including IP
              addresses, as part of that service. See <a className="underline" href="https://www.cloudflare.com/privacypolicy/">Cloudflare’s privacy policy</a>.
              This website does not add advertising pixels or visitor analytics scripts. Resource downloads do not subscribe you to marketing.
            </p>
            <p className="mt-md">
              The website does not save form drafts or assessment answers in browser storage. Hosting and form
              providers may process technical information to deliver and protect
              their services under their own policies.
            </p>
          </InfoCard>
          <InfoCard icon="public" title="Service providers and overseas processing">
            <p>
              Submitted forms are held in Formspree and notifications are delivered to our business email service.
              Email enquiries and replies are also handled through that email service. Formspree processing includes
              the United States; Formspree and Cloudflare may process information in other countries where they or
              their service providers operate. We do not promise that website enquiry information stays in Australia.
            </p>
            <p className="mt-md">
              Provider privacy policies linked on this page describe their processing. A linked external website
              operates under its own privacy arrangements.
            </p>
          </InfoCard>
          <InfoCard icon="history" title="Retention and deletion">
            <p>
              We retain enquiry records for as long as needed to respond, manage requested follow-up or an ongoing
              engagement, resolve issues and meet applicable legal record-keeping requirements. Our retention rule
              is to delete or de-identify records when they are no longer needed for these purposes. There is no automatic deletion promise when you
              close this website or clear a draft.
            </p>
            <p className="mt-md">
              You can ask us to delete an enquiry. We will assess the request across our form and email records and
              explain any information that must be retained and why. Project-specific retention is agreed separately.
            </p>
          </InfoCard>
          <InfoCard icon="contact_support" title="Access, correction and privacy concerns">
            <p>
              Email <a className="underline" href="mailto:hello@heutrix.com.au">hello@heutrix.com.au</a> with a privacy question,
              concern, or a request to access, correct or delete information you have sent. Include enough business
              context to identify the request, without sending sensitive records.
            </p>
            <p className="mt-md">
              We may need to confirm your identity or authority before releasing or changing information. We will
              review your concern, tell you if we need more information, and explain our response and any available
              next steps. You may also contact the{' '}
              <a className="underline" href="https://www.oaic.gov.au/privacy/privacy-complaints">Office of the Australian Information Commissioner</a>{' '}
              about privacy rights or a complaint within its jurisdiction.
            </p>
          </InfoCard>
          <InfoCard icon="update" title="Changes to this policy">
            <p>
              We may update this policy as website or information-handling arrangements change. The current version
              and update date will be available on this page. A new policy does not itself give permission for an
              unrelated use of information you have already provided.
            </p>
          </InfoCard>
        </div>
      </Section>
  ...
  return (
    <>
      <PageHero title="Terms of use" compact>
        <p>
          These terms apply to the Heutrix website, operated by Heutrix Pty Ltd
          (ABN 64 702 109 662; ACN 702 109 662), and its general information and resources.
        </p>
        <p>
      ...
      <Section className="bg-surface-container-low">
        <div className="grid gap-lg md:grid-cols-2">
          <InfoCard icon="rule" title="Service boundary">
            <p className="mb-md">Heutrix provides Diagnostics, Workflow Transformation and AI Guardrails.</p>
            <p className="mb-md">These services do not include:</p>
            <BulletList
              tone="boundary"
              items={[
                'Legal advice',
          ...
          <InfoCard icon="schedule" title="Introductory conversation">
            <p>
              “Talk to Heutrix” is a 20-minute, no-obligation introductory conversation. It is not a consulting workshop,
              technical design session or tailored professional advice, and it does not create an engagement.
            </p>
            <p className="mt-md">
              Review your request on the enquiry page, then choose to send it through Formspree. Reviewing does not
              send it. A successful submission confirms receipt of your request, not a booking; a time is agreed separately.
            </p>
          </InfoCard>
          ...
          <InfoCard icon="verified_user" title="Your rights under Australian law">
            <p>
              Nothing in these website terms excludes, restricts or modifies a consumer guarantee, remedy or other
              right under the Australian Consumer Law or another law that cannot lawfully be excluded. The website’s
              general-information limits do not remove those rights.
            </p>
          </InfoCard>
          <InfoCard icon="security" title="Appropriate website use">
            <p>
              Use the website lawfully. Do not interfere with its availability or security, attempt unauthorised
              access, send malicious code or spam, impersonate someone, or submit information you are not entitled
              to share. Referral details require the relevant person’s permission.
            </p>
            <p className="mt-md">
              Keep participant, patient, staff-sensitive and other confidential records out of public enquiries.
              Our <a className="underline" href="/privacy-and-data-handling">privacy policy and data-handling information</a>{' '}
              explains the collection and use of website information.
            </p>
          </InfoCard>
          <InfoCard icon="copyright" title="Website material and resource use">
            <p>
              Respect the copyright, branding and any licence or use conditions shown on website material and
              downloads. Do not misrepresent Heutrix material as your own work or imply Heutrix has endorsed your
              organisation, modified resource or implementation. Third-party materials retain their own rights and
              licence conditions.
            </p>
          </InfoCard>
          <InfoCard icon="link" title="External services and changes">
            <p>
              External websites and services have their own terms and privacy arrangements. A link does not mean
              Heutrix controls that service or guarantees its information. Check the relevant terms before using it.
            </p>
            <p className="mt-md">
              Website content, features and these terms may change. The current version and update date appear on
              this page. Website updates do not alter an existing agreed project scope or remove accrued legal rights.
            </p>
          </InfoCard>
          <InfoCard icon="gavel" title="Website operator">
            <p>
              This website is operated by Heutrix Pty Ltd (ABN 64 702 109 662; ACN 702 109 662).
              For questions about website use, contact <a className="underline" href="mailto:hello@heutrix.com.au">hello@heutrix.com.au</a>.
            </p>
          </InfoCard>
        </div>
      </Section>
      ...
      <Section className="bg-surface-container-low">
        <div className="mx-auto max-w-4xl rounded-xl border border-outline-variant bg-white p-xl shadow-sm">
          ...
          <p className="mb-md font-body-lg text-body-lg text-on-surface-variant">
            Heutrix does not provide audit certification, legal advice, clinical advice, privacy advice, regulatory
          ...
          <div className="mt-lg grid gap-md md:grid-cols-2">
            ...
            <InfoCard icon="gavel" title="Website operator">
              <p>
                Heutrix is operated by Heutrix Pty Ltd (ABN 64 702 109 662).
                For questions about this website or its information, contact hello@heutrix.com.au.
              </p>
            </InfoCard>
          </div>
        </div>
      ...
      <PageHero
        ...
        actions={(
          <>
            ...
            <ButtonLink href="/services" variant="secondary" icon={null}>Explore Services</ButtonLink>
          </>
        )}
        compact
      >
        <p>Use the current navigation to explore Heutrix services, free resources or the enquiry form.</p>
      </PageHero>
    </>
  );
}
...
function CtaSection({ title, body, cta = ctas.fitCall }) {
  return (
    <Section className="bg-primary text-on-primary">
      <motion.div className="grid gap-lg lg:grid-cols-[1fr_auto] lg:items-center" variants={stagger} initial={false} whileInView="show" viewport={{ once: true, amount: 0.2 }}>
