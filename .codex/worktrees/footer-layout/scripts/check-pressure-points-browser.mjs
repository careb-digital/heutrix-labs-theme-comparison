...
const errors = [];
try {
  for (const [width, height, reducedMotion] of [[1440,1000,'no-preference'],[390,844,'no-preference'],[320,700,'no-preference'],[320,700,'reduce']]) {
    ...
    assert.ok(await page.locator('.pressure-navigation button').evaluateAll(buttons => buttons.every(button => button.getBoundingClientRect().width >= 24)), 'Carousel touch targets must remain at least 24px wide');
    if (width === 390) {
      await page.emulateMedia({reducedMotion: 'reduce'});
      await page.waitForFunction(() => !document.querySelector('.pressure-play'));
      await page.emulateMedia({reducedMotion: 'no-preference'});
      await page.locator('.pressure-play').waitFor();
    }
