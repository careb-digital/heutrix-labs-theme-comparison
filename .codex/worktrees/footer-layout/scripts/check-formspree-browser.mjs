...
try {
  for (const width of [1440, 390, 320]) {
    ...
    async function snapshot() {
      return page.locator('form[aria-labelledby="enquiry-start-title"] [name]:not([type="hidden"])').evaluateAll(fields => Object.fromEntries(fields.map(f => [f.name, f.type === 'checkbox' ? f.checked : f.value])));
    }
    async function privacyBack() {
      ...
      await page.goBack();
      await page.locator('#enquiry-name').waitFor();
      assert.deepEqual(await snapshot(), before, 'All fields and checked/unchecked choices survive privacy and Back');
    }
    for (const referral of [false, true]) {
      ...
      await consent.uncheck(); await privacyBack(); await review.click();
      assert.equal(await page.locator('#enquiry-draft').count(), 0);
      await consent.check();
      await privacyBack();
      const beforeNavigation = await snapshot();
      await page.locator('.lm-contact-nav a').nth(referral ? 0 : 1).click();
      assert.equal(await page.locator('#enquiry-name').inputValue(), '', 'Other draft type starts empty');
      await page.locator('#enquiry-name').fill('Other synthetic draft');
      await page.locator('.lm-contact-nav a').nth(referral ? 1 : 0).click();
      assert.deepEqual(await snapshot(), beforeNavigation, 'Draft types are isolated');
      ...
      await page.locator('.lm-contact-nav a').nth(referral ? 0 : 1).click();
      assert.equal(await page.locator('#enquiry-name').inputValue(), 'Other synthetic draft', 'Success preserves the other draft');
      await page.getByRole('button', {name: 'Discard draft', exact: true}).click();
      await page.locator('.lm-contact-nav a').nth(referral ? 1 : 0).click();
      assert.equal(await page.locator('#enquiry-name').inputValue(), '', 'Successful draft clears on return');
      assert.equal(await page.locator('[name="contactPermission"]').isChecked(), false);
    }
    if (width === 1440) {
      ...
      await privacyBack();
      await page.locator('.lm-contact-nav a').nth(1).click();
      await page.locator('.lm-contact-nav a').nth(0).click();
      assert.equal(await page.locator('[name="includeSummary"]').isChecked(), false, 'Summary opt-out survives referral navigation and plain contact URL');
      await page.getByRole('button', {name: 'Review My Enquiry'}).click();
      responseMode = 'success';
      await page.getByRole('button', {name: 'Send Enquiry', exact: true}).click();
      await page.locator('[aria-label="Submission confirmation"]').waitFor();
    ...
    // A delayed success must clear a restored form and prevent duplicate review/send.
    await page.goto(origin + '/contact');
    await fill(false);
    await page.getByRole('button', {name: 'Review My Enquiry'}).click();
    responseMode = 'pending';
    // Hover near the top edge: a translating button can oscillate under the
    // pointer, making the real action and Playwright's click unstable.
    const sendButton = page.getByRole('button', {name: 'Send Enquiry', exact: true});
    const sendBounds = await sendButton.boundingBox();
    await page.mouse.move(sendBounds.x + 5, sendBounds.y + 0.5);
    const positions = await sendButton.evaluate(async button => {
      const ys = [];
      for (let frame = 0; frame < 8; frame++) {
        await new Promise(resolve => requestAnimationFrame(resolve));
        ys.push(button.getBoundingClientRect().y);
      }
      return ys;
    });
    assert.ok(Math.max(...positions) - Math.min(...positions) < 1, 'Send action must remain stationary under an edge hover');
    await page.getByRole('button', {name: 'Send Enquiry', exact: true}).click();
    await page.waitForFunction(() => document.querySelector('form[aria-busy="true"]'));
    for (let i = 0; !release && i < 250; i++) await new Promise(resolve => setTimeout(resolve, 20));
    assert.ok(release);
    await page.locator('.lm-contact-nav a').nth(1).click();
    await page.locator('.lm-contact-nav a').nth(0).click();
    assert.ok(await page.getByRole('button', {name: 'Review My Enquiry'}).isDisabled());
    responseMode = 'success'; release(); release = undefined;
    await page.locator('[aria-label="Submission confirmation"]').waitFor();
    await page.locator('.lm-contact-nav a').nth(1).click();
    await page.locator('.lm-contact-nav a').nth(0).click();
    await page.waitForFunction(() => document.querySelector('#enquiry-name').value === '');
    assert.equal(await page.locator('[name="contactPermission"]').isChecked(), false);
    await fill(false);
    await page.getByRole('button', {name: 'Review My Enquiry'}).click();
    await page.getByRole('button', {name: 'Discard draft', exact: true}).click();
    ...
    await fill(false);
    await page.reload();
    assert.equal(await page.locator('#enquiry-name').inputValue(), '', 'Reload starts a fresh draft');
