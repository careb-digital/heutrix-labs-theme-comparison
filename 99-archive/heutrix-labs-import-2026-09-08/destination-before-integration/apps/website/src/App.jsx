...
function PageRenderer({ path, search }) {
  ...
  switch (path) {
    ...
    case '/refer':
      return <EnquiryPage key="referral" kind="referral" search={search} />;
  ...
  return (
    ...
    >
      ...
      <div className="mb-lg flex flex-col gap-sm sm:flex-row sm:flex-wrap">
        <ButtonLink href={`/resources/${resource.id}`}>Start the interactive tool</ButtonLink>
    ...
    <Section className="bg-surface-container-low">
      <div className="grid gap-xl lg:grid-cols-[0.75fr_1.25fr] lg:items-start">
        <SectionIntro eyebrow={eyebrow} title="Work through the problem before you request a call.">
          ...
          <p>
            Use the interactive tool, then save your action plan or discuss it with Heutrix. The guide and workbook are also available without an email address.
          </p>
        </SectionIntro>
  ...
  return (
    <>
        ...
        <div className="mx-auto grid max-w-container-max gap-xl lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <motion.div className="max-w-3xl" variants={stagger} initial="hidden" animate="show">
            ...
            <motion.div variants={fadeUp} className="flex flex-col gap-md sm:flex-row sm:flex-wrap">
              ...
              <ButtonLink href="/resources/workflow-bottleneck-scorecard" variant="secondary" icon={null}>Try the workflow scorecard</ButtonLink>
      ...
      <Section id="home-services" className="bg-surface">
        <SectionIntro eyebrow="Three starting points" title="Start with the smallest useful engagement." className="mx-auto mb-xl text-center">
          <p>
            Use Diagnostics when the priority is unclear, Workflow Transformation when one defined workflow is ready
            to improve, or AI Guardrails when workplace AI needs practical boundaries.
          </p>
        </SectionIntro>
        <motion.div className="grid gap-lg md:grid-cols-2 xl:grid-cols-[0.9fr_1.1fr_0.9fr]" variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.08 }}>
      ...
      <HomeFitAndBoundarySection />

      <CtaSection
        title="Bring one workflow that should not be this hard."
        body="You do not need a process map, technical brief or proposed solution. Bring a general, non-sensitive description of what starts the work, who is involved and where it becomes difficult to track, hand over or report."
      />
    </>
  );
}
        ...
        <div className="rounded-xl border border-outline-variant bg-white p-xl shadow-sm">
          <div className="max-w-4xl">
            ...
            <p className="mb-md font-body-lg text-body-lg text-on-surface-variant">
              Heutrix Pty Ltd (ABN 64 702 109 662; ACN 702 109 662) is verified against ABR. Remaining address, insurance, service-area details and
      ...
      <Section>
        <div className="grid gap-lg lg:grid-cols-3">
          <InfoCard icon="contact_page" title="Information for the workflow fit call">
            ...
            <p className="mt-md">
              Interactive assessments and enquiry drafts stay in page memory. They are not saved to browser storage or sent automatically. If you submit a test request, the demo service validates and discards it; no contact is stored, no message is sent and no call is booked. You can review a selected assessment summary, choose whether to include it, and send a draft through your email app. If a connected form or calendar is available, you complete the request there. A draft is not a received enquiry or a confirmed booking.
            </p>
          </InfoCard>
