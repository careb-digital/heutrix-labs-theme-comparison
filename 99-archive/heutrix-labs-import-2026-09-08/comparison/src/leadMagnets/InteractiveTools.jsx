...
export function ResourceHub() {
  return <div className="r-page lm-page"><section className="r-section r-page-hero"><div className="r-container">
    ...
    <div className="lm-tool-grid">{tools.map((tool, i) => {const Icon = icons[tool.icon]; return <article className="lm-tool-card" key={tool.id}>
      ...
      <a className="r-button" href={`/resources/${tool.id}`}>{i === 0 ? 'Get the Scorecard' : 'Take the Assessment'} <ArrowRight size={17}/></a>
      <div className="r-download-links"><a href={resources[i].guideHref} download>Get the Guide</a><a href={resources[i].workbookHref} download>Get the Template</a></div>
    </article>})}</div>
...
function ToolResult({tool, answers, onEdit}) {
  ...
  return <div className="lm-result">
    ...
    <aside className="lm-result-side"><div className="lm-panel lm-next-step"><p className="r-eyebrow">Take it into the next conversation</p><h2>{outcome.service ? 'Talk through this result.' : 'Keep the next step practical.'}</h2>
      <p>{outcome.service ? `A free, 20-minute conversation can help establish whether ${outcome.service === 'heutrix-diagnostics' ? 'Heutrix Diagnostics' : outcome.service === 'heutrix-ai-guardrails' ? 'Heutrix AI Guard' : 'Heutrix Workflow Transformation'} is relevant.` : 'Try the suggested action with your team. If the problem persists, bring this summary to a conversation.'}</p>
      <a className="r-button" href={`/contact?service=${outcome.service || 'not-sure'}&source=${tool.id}`} onClick={prepare}>{outcome.service === 'heutrix-diagnostics' ? 'Discuss Diagnostics' : outcome.service === 'heutrix-workflow-transformation' ? 'Discuss Transformations' : outcome.service === 'heutrix-ai-guardrails' ? 'Discuss AI Guard' : 'Talk to Heutrix'} <ArrowRight size={17}/></a><p className="lm-hint">Review the summary before sharing. No obligation to start a project.</p>
