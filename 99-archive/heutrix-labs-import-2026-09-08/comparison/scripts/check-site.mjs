...
if(origin){
 ...
 for(const path of paths){const r=await fetch(origin+path);assert.equal(r.status,200,path);assert.match(r.headers.get('content-type'),/text\/html/);assert.match(await r.text(),/<div id="root"><\/div>/,path+' missing application shell');assert.equal(r.headers.get('x-robots-tag'),null);}
...
console.log('PASS: three illustrative workflow examples, six approved resource hashes and explicit consultation email path');
